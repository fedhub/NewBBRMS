$(document).ready(function() {
    ...
    $('#save-details').click(function(){
        ...
        else add_addition_type_ajax(data);
    });
...
function init(){
    ...
    var id_params = $('.add-addition-type-cont').attr('id').split('-');
    return {
...
function check_inputs(data){
    ...
    if(!(/^\d+$/).test(data.addition_item_price)) msg += 'מחיר צריך להיות מורכב רק מספרות. ';
    return msg;
}

function add_addition_type_ajax(data){
    var url = base_url + '/add-addition-type';
    $.ajax({
        url: url,
        type: 'POST',
        data: {data: JSON.stringify(data)}
    }).done(function(res){
        if(res.status) window.location = base_url + '/menu-additions&menu_type_id='+data.menu_type_id+'&menu_type_name='+data.menu_type_name+'&menu_item_id='+data.menu_item_id+'&menu_item_name='+data.menu_item_name;
        else alert(res.msg);
    })
}
